...
public record PublishersCreatedRecordDh(
        ...
        @Pattern(
                regexp = "^[\\p{L}0-9\\s\\.,'’\\-\u00B7]*$",
                message = "Publisher Address contains invalid characters")
        String address,

        @Size(max = 40, message = "City cannot exceed 40 characters")
        @Pattern(
                regexp = "^[\\p{L}0-9\\s\\.,'’\\-\u00B7]*$",
                message = "Publisher City contains invalid characters")
        String city,

        @Size(max = 30, message = "Province cannot exceed 30 characters")
        @Pattern(regexp = "^[\\p{L}0-9\\s\\.,'’\\-\u00B7]*$",
                message = "Publisher Province contains invalid characters")
        String province,
        ...
        @Pattern(
                regexp = "^[A-Za-z0-9\\s-]+$",
                message = "Publisher Postal code must contain only numbers")
        String postalCode,

        @Size(max = 30, message = "Country cannot exceed 30 characters")
        @Pattern(
                regexp = "^[\\p{L}0-9\\s\\.,'’\\-\u00B7]*$",
                message = "Publisher Country contains invalid characters")
        String country,
        ...
        @Pattern(
                regexp = "^[0-9+\\-\\s()]{6,16}$",
                message = "Publisher Phone number contains invalid characters (letters are not allowed)")
        String phone,

        @Size(max = 255, message = "Publisher Notes cannot exceed 255 characters")
        String notes
)
{}
